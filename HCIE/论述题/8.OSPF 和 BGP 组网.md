...
`bestroute igp-metric-ignore` 的作用是使 BGP 在选择最优路由时忽略下一跳 IGP 路由的度量值

以 R3 为例，R3 上如果不开启忽略比较下一跳 IGP 开销，那么就算开启负载分担，也不会生效。

因为负载分担需要前 9 条都一致，才可以实现

如图所示， R3 去往 R1、R2 的 IGP cost 不一样，所以需要忽略比较下一跳 IGP 开销，才可以实现 BGP 负载。
...
但是因为 BGP 的 `holdtime` 时间默认为 180s，所以 R1-R3 邻居检测到故障，需要等到 3min 的 `holdtime `超时之后。 在没有检测到故障之前，邻居依旧正常，路由不变。3min 之后，邻居故障，路由只剩下 R2 作为下一跳的一个路径。
