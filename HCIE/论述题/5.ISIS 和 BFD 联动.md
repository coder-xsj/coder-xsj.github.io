...
​	BFD的detect-multiplier 采用默认值3，是不是min-rx-interval 和min-tx-interval参数越小越好？如何合理的设置min-rx-interval 和 min-tx-interval的取值范围？（4分）
 ...
 `min-rx-interval` 和 `min-tx-interval` 并不是配置的参数值越小。

​	若 `min-rx-interval` 和 `min-tx-interval` 配置的参数值越小，BFD 发送报文频率越高，对当前网络感知速度越快，适合于链路质量较高的链路，对于不太稳定的链路，如果 BFD 检测时间较小，则 BFD 会话可能会发生震荡，这时可以根据实际网络情况调整 BFD 检测时间。
